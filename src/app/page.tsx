...
export default function Home() {
  return <div className="home-page">
    ...
    <section className="dark-section products-section"><Container><SectionHeading eyebrow="Produtos" title="Produtos que já estão em produção." center /><div className="product-grid">{products.map(x => <Link href="/produtos" className="product-card" key={x[0]}><div className="product-art" style={{ "--accent": x[3] } as React.CSSProperties}><span>{x[0].slice(0, 1)}</span><i /><i /><i /></div><h3>{x[0]}</h3><p>{x[1]}</p><small>{x[2]}</small><span className="text-link">Saiba mais ↗</span></Link>)}</div></Container></section>
